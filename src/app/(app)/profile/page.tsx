...
import { User, MapPin, Star, Leaf, DollarSign } from 'lucide-react'
...
export default async function ProfilePage() {
  ...
  const { data: actions } = await supabase
    .from('user_actions')
    .select('actions_library(co2_saved_kg, money_saved_nzd)')
    .eq('user_id', user.id)
        ...
        {[
          {
            icon: User,
            label: 'Account type',
            value: profile?.user_type ? USER_TYPE_LABELS[profile.user_type] ?? profile.user_type : 'Not set',
          },
          {
            icon: MapPin,
            label: 'Region',
          ...
          ...(profile?.groups ? [{
            icon: User,
            label: 'Group',
            value: (profile.groups as unknown as { name: string } | null)?.name ?? '—',
          }] : []),
        ].map((row) => (
